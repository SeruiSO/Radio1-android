...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function initializeApp() {
    ...
    tabsContainer.addEventListener("mousedown", handleLongPress);
    tabsContainer.addEventListener("touchstart", handleLongPress);
    ...
    audio.addEventListener("error", () => {
      console.error("Помилка відтворення аудіо:", audio.error);
      errorCount++;
    ...
    window.addEventListener("online", () => {
      console.log("Пристрій онлайн: пробуємо відновити відтворення");
    ...
    window.addEventListener("offline", () => {
      console.log("Пристрій офлайн: зупиняємо спроби відтворення");
  ...
  function renderCustomTabs() {
    ...
    customTabs.forEach(tab => {
      if (!document.querySelector(`.tab-btn[data-tab='${tab}']`)) {
        ...
        searchBtn.parentNode.insertBefore(tabBtn, searchBtn);
      }
    });
  }
  ...
  function showCreateTabModal() {
    if (customTabs.length >= 10) {
      alert("Досягнуто ліміт у 10 вкладок!");
      return;
    }
    const overlay = document.createElement("div");
    overlay.className = "modal-overlay";
    const modal = document.createElement("div");
    modal.className = "modal-create-tab";
    modal.innerHTML = `
      <h2>Нова вкладка</h2>
      <div class="modal-create-tab-content">
        <input type="text" maxlength="10" pattern="[A-Za-z]+" title="Тільки латинські літери, максимум 10 символів">
        <button class="modal-create-btn">Підтвердити</button>
    ...
    createBtn.addEventListener("click", () => {
      ...
      if (!/^[A-Za-z]+$/.test(tabName)) {
        alert("Назва вкладки має містити тільки латинські літери!");
        return;
      }
      if (tabName.length > 10) {
        alert("Назва вкладки не може перевищувати 10 символів!");
        return;
      }
      const allTabs = ["best", "techno", "trance", "ukraine", "pop", "search", ...customTabs];
  ...
  function handleLongPress(e) {
    const tabBtn = e.target.closest(".tab-btn");
    if (!tabBtn || !customTabs.includes(tabBtn.dataset.tab)) return;
    e.preventDefault();
    clearLongPress();
    longPressTimer = setTimeout(() => {
      showEditTabModal(tabBtn.dataset.tab);
      longPressTimer = null;
    }, 500);
  }
  ...
  function showEditTabModal(tabName) {
    ...
    overlay.className = "modal-overlay";
    const modal = document.createElement("div");
    modal.className = "modal-edit-tab";
    modal.innerHTML = `
      <h2>Редагувати вкладку</h2>
      <div class="modal-edit-tab-content">
        <button class="modal-rename-btn">Перейменувати</button>
        <button class="modal-delete-tab-btn">Видалити</button>
        <button class="modal-cancel-btn">Відміна</button>
      </div>
    `;
    document.body.appendChild(overlay);
    document.body.appendChild(modal);
    const renameBtn = modal.querySelector(".modal-rename-btn");
    const deleteBtn = modal.querySelector(".modal-delete-tab-btn");
    const cancelBtn = modal.querySelector(".modal-cancel-btn");
    const closeModal = () => {
      overlay.remove();
      modal.remove();
  ...
  function showRenameTabModal(tabName) {
    ...
    modal.innerHTML = `
      ...
      <div class="modal-create-tab-content">
        <input type="text" maxlength="10" pattern="[A-Za-z]+" value="${tabName}" title="Тільки латинські літери, максимум 10 символів">
        <button class="modal-create-btn">Підтвердити</button>
        <button class="modal-cancel-btn">Відміна</button>
      </div>
    `;
    ...
    createBtn.addEventListener("click", () => {
      ...
      if (!/^[A-Za-z]+$/.test(newTabName)) {
        alert("Назва вкладки має містити тільки латинські літери!");
        return;
      }
      if (newTabName.length > 10) {
        alert("Назва вкладки не може перевищувати 10 символів!");
        return;
      }
      const allTabs = ["best", "techno", "trance", "ukraine", "pop", "search", ...customTabs];
      if (allTabs.includes(newTabName) && newTabName !== tabName) {
        alert("Вкладка з такою назвою вже існує!");
        return;
      }
  ...
  function renameTab(oldName, newName) {
    ...
    const tabBtn = document.querySelector(`.tab-btn[data-tab='${oldName}']`);
    if (tabBtn) {
  ...
  function deleteTab(tabName) {
    ...
    document.querySelector(`.tab-btn[data-tab='${tabName}']`)?.remove();
    if (currentTab === tabName) {
      const allTabs = ["best", "techno", "trance", "ukraine", "pop", ...customTabs];
    ...
    try {
      ...
      if (response.ok) {
        ...
        Object.keys(userAddedStations).forEach(tab => {
          if (!newStations[tab] && customTabs.includes(tab)) {
  ...
  function showTabModal(item) {
    ...
    modal.innerHTML = `
      ...
      <div class="modal-tabs">
        ...
        ${customTabs.map(tab => `<button class="modal-tab-btn" data-tab="${tab}">${tab}</button>`).join('')}
        <button class="modal-cancel-btn">Відміна</button>
      </div>
    `;
    document.body.appendChild(overlay);
  ...
  function switchTab(tab) {
    ...
    document.getElementById("searchInput").style.display = tab === "search" ? "flex" : "none";
    ...
    if (stationItems?.length && currentIndex < stationItems.length && intendedPlaying) {
      ...
      const normalizedAudioSrc = normalizeUrl(document.getElementById("audioPlayer").src);
      if (normalizedAudioSrc !== normalizedCurrentUrl || document.getElementById("audioPlayer").paused || document.getElementById("audioPlayer").error || document.getElementById("audioPlayer").readyState < 2 || document.getElementById("audioPlayer").currentTime === 0) {
  ...
  function updateStationList() {
    ...
    stations.forEach((station, index) => {
      ...
      const iconHtml = item.dataset.favicon ? `<img src="${station.favicon}" alt="${station.name} icon" style="width: 32px; height: 32px; object-fit: contain; margin-right: 10px;" onerror="this.outerHTML='🎵'; console.warn('Помилка favicon:', ...);" />` : "🎵 ";
    ...
    stationList.onclick = e => {
      ...
      if (deleteBtn) {
        e.stopPropagation();
        if (window.confirm(`Ви впевнені, що хочете видалити станцію "${item.dataset.name}" зі списку?`)) {
  ...
  function toggleFavorite(stationName) {
    ...
    localStorage.setItem("favoriteStations", JSON.stringify(favoriteStations));
    if (currentTab === "best") switchTab(currentTab);
  ...
  function updateCurrentStation(item) {
    ...
    const stationCountryElement = currentStationInfo.querySelector(".station-country");
    const stationIconElement = currentStationInfo.querySelector(".station-icon");
    if (!stationNameElement || !stationGenreElement || !stationCountryElement || !stationIconElement) {
      console.error("Один елемент currentStationInfo не знайдено", {
        stationNameElement: !!stationNameElement,
        stationGenreElement: !!stationGenreElement,
        stationCountryElement: !!stationCountryElement,
        stationIconElement: !!stationIconElement
      });
      return;
    }
    ...
    if (item.dataset.favicon) {
      stationIconElement.innerHTML = `<img src="${item.dataset.favicon}" alt="${item.dataset.name}" style="width: 100%; height: 100%; object-fit: contain;" onerror="this.outerHTML='🎵'; console.error('Помилка favicon:', '${item.dataset.favicon}');">`;
    } else {
      stationIconElement.textContent = "🎵";
      stationIconElement.style.backgroundImage = "";
    }
  ...
  function updateMediaSession() {
    if ("mediaSession" in navigator) {
      const currentStationInfo = document.getElementById("currentStationInfo");
      const stationName = currentStationInfo.querySelector(".station-name")?.textContent || "Radio S O";
      const stationGenre = currentStationInfo.querySelector(".station-genre")?.textContent.replace("genre: ", "") || "-";
      const stationCountry = currentStationInfo.querySelector(".station-country")?.textContent.replace("country: ", "") || "-";
      const favicon = currentStationInfo.querySelector(".station-icon img")?.src || "";
      navigator.mediaSession.metadata = new MediaMetadata({
        title: stationName,
        artist: `${stationGenre} | ${stationCountry}`,
        album: "Radio S O",

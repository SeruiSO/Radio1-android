...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!audio || !stationList || !playPauseBtn || !currentStationInfo.name || !artwork || !progressBar || !navBack || !navMenu || !shuffleBtn) {
  ...
  function initializeApp() {
    ...
    navMenu.addEventListener("click", () => switchTab(currentTab));
    ...
    function toggleTheme() {
      const themes = ["neon-pulse", "lime-surge", "flamingo-flash", "violet-vortex", "aqua-glow", "cosmic-indigo", "mystic-jade", "aurora-haze", "starlit-amethyst", "lunar-frost"];
    ...
    function tryAutoPlay() {
      ...
      audio.pause(); audio.src = ""; audio.src = stationItems[currentIndex].dataset.value;
      const playPromise = audio.play();
      playPromise.then(() => { errorCount = 0; playPauseBtn.textContent = "⏸"; }).catch(error => { console.error("Помилка відтворення:", error); errorCount++; });
    }

    function switchTab(tab) {
      if (!["techno", "trance", "ukraine", "pop"].includes(tab)) tab = "techno";
      currentTab = tab;
    ...
    function updateStationList() {
      let stations = stationLists[currentTab] || [];
      if (!stations.length) {
        currentIndex = 0;
        stationItems = [];
        stationList.innerHTML = `<div class='station-item empty'>Немає станцій у цій категорії</div>`;
        return;
      }
    ...
    function updateCurrentStationInfo(item) {
      ...
      artwork.style.backgroundImage = `url('icon-192.png')`; // Замінити на динамічну іконку, якщо доступна

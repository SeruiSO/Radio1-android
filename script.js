/* Radio S O - Cosmic Stream JavaScript */
...
const stationList = document.getElementById("stationList") || document.querySelector(".station-list");
...
const waveCircles = currentStationInfo?.querySelectorAll(".wave-circle");
...
const searchBtn = searchInput?.querySelector(".search-btn");
...
function initializeApp() {
  ...
  // Gesture Support
  stationList.addEventListener("touchstart", handleTouchStart, false);
  stationList.addEventListener("touchmove", handleTouchMove, false);
  stationList.addEventListener("touchend", handleTouchEnd, false);
  ...
  function handleTouchEnd(e) {
    ...
    const index = Array.from(stationList.children).indexOf(stationItem);
    const stationName = stationItems[index]?.dataset.name;

    const swipeDistance = touchEndX - touchStartX;
    if (swipeDistance > 50 && stationName) {
      toggleFavorite(stationName);
    } else if (swipeDistance < -50 && stationName) {
      if (confirm(`Are you sure you want to delete station "${stationName}" from the list?`)) {
  ...
  applyTheme(localStorage.getItem("selectedTheme") || "cosmic-neon");
  loadStations();
...
function exportSettings() {
  const settings = {
    selectedTheme: localStorage.getItem("selectedTheme") || "cosmic-neon",
    customTabs,
    userAddedStations,
    favoriteStations,
    pastSearches,
...
function importSettings(event) {
  ...
  reader.onload = (e) => {
    try {
      ...
      const validThemes = [
        "cosmic-neon", "dark-abyss", "emerald-glow", "retro-wave",
        "lime-surge", "flamingo-flash", "aqua-glow", "aurora-haze",
...
function populateSearchSuggestions() {
  ...
  const countryDatalist = document.getElementById("country-suggestions");
...
function resetStationInfo() {
  if (stationName) stationName.textContent = "Select station";
  if (stationGenre) stationGenre.textContent = "genre: -";
  if (stationCountry) stationCountry.textContent = "country: -";
  if (stationIcon) {
    stationIcon.innerHTML = "🎵";
    stationIcon.style.backgroundImage = "none";
    stationIcon.classList.remove("playing");
  }
  waveCircles?.forEach(circle => circle.classList.remove("playing"));
}
...
function renderSearchResults(stations) {
  ...
  stations.forEach((station, index) => {
    ...
    const iconHtml = item.dataset.favicon ? `<img src="${item.dataset.favicon}" alt="${station.name} icon" style="width: 36px; height: 36px; object-fit: contain;" onerror="this.outerHTML='🎵 '">` : "🎵 ";
    item.innerHTML = `${iconHtml}<span class="station-name">${station.name}</span><div class="buttons-container"><button class="add-btn">➕</button></div>`;
...
function showTabModal(item) {
  ...
  modalTabs.querySelector(".modal-cancel-btn").addEventListener("click", closeModal, { once: true });
}
...
function showEditTabModal(tab) {
  ...
  const saveBtn = modal.querySelector(".modal-tab-btn");
  const cancelBtn = modal.querySelector(".modal-cancel-btn");
  ...
  saveBtn.addEventListener("click", renameTabHandler, { once: true });
  cancelBtn.addEventListener("click", closeModal, { once: true });
  modalOverlay.addEventListener("click", closeModal, { once: true });
  input.addEventListener("keypress", (e) => {
...
let currentThemeIndex = themes.indexOf(localStorage.getItem("selectedTheme") || "cosmic-neon");
...
function applyTheme(theme) {
  ...
  themeToggle.innerHTML = theme === "cosmic-neon" ? "🌙" : "🌞";
}
  ...
  try {
    ...
    const attemptPlay = async (attemptsLeft) => {
      ...
      try {
        ...
        waveCircles?.forEach(circle => circle.classList.add("playing"));
...
function updateStationList() {
  ...
  stations.forEach((station, index) => {
    ...
    item.dataset.favicon = station.favicon && isValidUrl(station.favicon) ? station.favicon : "";
    const iconHtml = item.dataset.favicon ? `<img src="${item.dataset.favicon}" alt="${station.name} icon" style="width: 36px; height: 36px; object-fit: contain;" onerror="this.outerHTML='🎵 '; console.warn('Error loading favicon:', '${item.dataset.favicon}');">` : "🎵 ";
...
function updateCurrentStation(item) {
  ...
  if (stationGenre) stationGenre.textContent = item.dataset.genre || "";
  if (stationCountry) stationCountry.textContent = item.dataset.country || "";
  if (stationIcon) {
    ...
    if (isPlaying) stationIcon.classList.add("playing");
    else stationIcon.classList.remove("playing");
  }
  if (isPlaying) waveCircles?.forEach(circle => circle.classList.add("playing"));
  else waveCircles?.forEach(circle => circle.classList.remove("playing"));

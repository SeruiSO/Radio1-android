...
let abortController = null; // Для скасування запитів
...
function resetStationInfo() {
  ...
  if ("mediaSession" in navigator) {
    navigator.mediaSession.metadata = null; // Очищення MediaSession
  }
}
...
async function loadStations() {
  ...
  try {
    ...
    if (error.name === "AbortError") {
      console.log("Запит до stations.json скасовано");
    } else {
    ...
    abortController = null; // Очищення контролера після завершення
  }
}
...
function tryAutoPlay() {
  ...
  // Перевірка, чи аудіо вже відтворюється і чи збігається джерело
  if (!audio.paused && audio.src === stationItems[currentIndex].dataset.value) {
    console.log("Аудіо вже відтворюється з правильним джерелом, пропускаємо");
    return;
  }
  // Перевірка валідності URL
  if (!isValidUrl(stationItems[currentIndex].dataset.value)) {
    console.warn(`Невалідна URL для станції ${stationItems[currentIndex].dataset.name}, переходимо до наступної`);
    nextStation();
    return;
  }
  isAutoPlaying = true;
  ...
  playPromise
    ...
    .catch(error => {
      ...
      isAutoPlaying = false;
      document.querySelectorAll(".wave-bar").forEach(bar => bar.style.animationPlayState = "paused");
      nextStation(); // Переходимо до наступної станції у разі помилки
    });
}
...
function updateStationList() {
  ...
  let stations = currentTab === "best"
    ? favoriteStations
        ...
        .filter(s => s) // Фільтруємо undefined (станції, яких немає в stations.json)
...
function toggleFavorite(stationName) {
  ...
  if (currentTab === "best") {
    // Оновлюємо список лише для вкладки "best"
    const prevIndex = currentIndex;
    updateStationList();
    currentIndex = prevIndex < stationItems.length ? prevIndex : 0;
    if (stationItems.length && currentIndex < stationItems.length) {
      changeStation(currentIndex); // Відновлюємо поточну станцію
    }
  } else {
    // Оновлюємо лише зірочки, не змінюючи список
    stationItems.forEach(item => {
      const btn = item.querySelector(".favorite-btn");
      if (btn) {
        btn.classList.toggle("favorited", favoriteStations.includes(item.dataset.name));
      }
    });
  }
}
...
function changeStation(index) {
  ...
  if (!isValidUrl(item.dataset.value)) {
    console.warn(`Невалідна URL для станції ${item.dataset.name}, переходимо до наступної`);
    nextStation();
    return;
  }
...
function togglePlayPause() {
  ...
  if (audio.paused) {
    ...
    document.querySelectorAll(".wave-bar").forEach(bar => bar.style.animationPlayState = "paused");
    if ("mediaSession" in navigator) {
      navigator.mediaSession.metadata = null; // Очищення MediaSession при паузі
    }
  }

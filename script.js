...
let userAddedStations = JSON.parse(localStorage.getItem("userAddedStations")) || {}; // New storage for user-added stations
let stationItems = [];
  ...
  function initializeApp() {
    ...
    searchBtn.addEventListener("click", () => {
      ...
      if (query || country || genre) {
        if (query && !pastSearchesList.includes(query)) {
    ...
    async function loadStations() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // Merge with user-added stations
      ...
      navigator.serviceWorker.addEventListener("message", (event) => {
        if (event.data.type === "CACHE_UPDATED") {
          ...
          if (currentCacheVersion !== event.data.cacheVersion) {
            ...
            loadStations(); // Trigger loadStations to merge user-added stations
          }
        }

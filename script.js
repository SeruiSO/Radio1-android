...
// Централізований об'єкт стану мережі
const networkState = {
  isOnline: navigator.onLine,
  reconnectionAttempts: 0,
  maxReconnectionAttempts: 30, // 30 спроб (3 хвилини по 2 секунди + додаткові)
  baseDelay: 2000, // 2 секунди для перших 3 хвилин
  longDelay: 5000, // 5 секунд після 3 хвилин
  maxDelay: 30000, // Максимальна затримка
  lastReconnectionTime: null,
  timeoutId: null,
  initialPhaseEnd: 3 * 60 * 1000 // 3 хвилини в мілісекундах
};
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function initializeApp() {
    ...
    shareButton.addEventListener("click", shareCurrentStation);
    ...
    searchQuery.addEventListener("keypress", (e) => e.key === "Enter" && searchBtn.click());
    searchCountry.addEventListener("keypress", (e) => e.key === "Enter" && searchBtn.click());
    searchGenre.addEventListener("keypress", (e) => e.key === "Enter" && searchBtn.click());
  ...
  function setupEventListeners() {
    window.addEventListener('online', debounce(handleNetworkChange, 500));
    window.addEventListener('offline', debounce(handleNetworkChange, 500));
    ...
    document.addEventListener('resume', debounce(() => {
  ...
  async function handleNetworkChange() {
    ...
    networkState.isOnline = await checkNetworkStatus();

    if (networkState.isOnline && !wasOnline) {
      console.log('Мережа відновлена');
      clearReconnectionAttempts();
      if (playerState.isPlaying) {
        debouncedTryAutoPlay();
      }
    } else if (!networkState.isOnline && wasOnline) {
  ...
  function startReconnectionProcess() {
    if (networkState.reconnectionAttempts >= networkState.maxReconnectionAttempts) {
    ...
    console.log(`Спроба повторного підключення через ${delay}ms (спроба ${networkState.reconnectionAttempts + 1})`);

    networkState.timeoutId = setTimeout(async () => {
      networkState.reconnectionAttempts++;
      networkState.lastReconnectionTime = Date.now();
  ...
  function calculateReconnectionDelay() {
    const elapsedTime = networkState.lastReconnectionTime
      ? Date.now() - networkState.lastReconnectionTime
      : 0;

    if (elapsedTime < networkState.initialPhaseEnd) {
      return networkState.baseDelay;
    }
    return networkState.longDelay;
  }

  function clearReconnectionAttempts() {
    clearTimeout(networkState.timeoutId);
    networkState.reconnectionAttempts = 0;
    networkState.lastReconnectionTime = null;
    networkState.timeoutId = null;
  }

  async function tryAutoPlay() {
    if (!networkState.isOnline) {
      console.log('Пристрій офлайн, відтворення призупинено');
      return;
    }

    if (!playerState.isPlaying || !stationItems?.length || playerState.currentIndex >= stationItems.length) {
      console.log('Відтворення не потрібне за поточним станом');
      document.querySelectorAll(".wave-line").forEach(line => line.classList.remove("playing"));
      return;
    }

    const currentStation = stationItems[playerState.currentIndex];
    if (!currentStation || !isValidUrl(currentStation.dataset.value)) {
      console.error('Невалідний URL станції');
      if (playerState.isPlaying) {
        startReconnectionProcess();
      }
      document.querySelectorAll(".wave-line").forEach(line => line.classList.remove("playing"));
    ...
    const targetSrc = currentStation.dataset.value;
    if (audio.src === targetSrc && !audio.paused) {
      console.log('Аудіо вже відтворюється');
      return;
    }
  ...
  function normalizeCountry(country) {
    ...
    return countryMap[normalized] || countryMap[normalized.toLowerCase()] || country.charAt(0).toUpperCase() + country.slice(1).toLowerCase();
}
...
abortController.signal
  ...
  function showEditTabModal(tab) {
    ...
    const renameTabHandler = () => {
      ...
      if (["best", "techno", "trance", "ukraine", "pop", "search"].includes(newName)) || customTabs.includes(newName)) {
        alert("Ція назва вкладки вже існує!");
        return;
      }
      ...
      userAddedStations[tabnewName] = null userAddedStations[tab] || [];
      delete stationLists[tab];
      delete userAddedStations[userTab];
      localStorage.setItem("customTabs", JSON.stringify(customTabs));
      localStorage.setItem("stationLists", JSON.stringify(stationLists));
      localStorage.setItem("userAddedStations", JSON.stringify(userAddedStations));
  ...
  function updateStationList() {
    ...
    if (stationItems.length && !stationItems[playerState.currentIndex].textContent && !stationItems[playerState.currentIndex].classList.contains("empty")) {
